...
# lajes_app.py - SISTEMA COMPLETO COM ROMANEIO MELHORADO E CHECK LIST
import os
import math
import shutil
import subprocess
...
class LajesApp(tk.Tk):
    ...
    def processar_v2(self):
        ...
        try:
            from core.lajes_motor import processar_lajes as motor_v2
        except Exception as e:
            messagebox.showerror(
                "Erro",
                f"Motor DXF 2.0 não encontrado!\ncore/lajes_motor.py ausente.\n{e}"
            )
            return  # << CORRETO
    ...
    def gerar_romaneio_conferencia(self):
        ...
        janela_conf.title("📋 CHECK LIST - Conferência de Lajes")
        ...
        tk.Label(
            header_frame,
            text="📋 CHECK LIST - CONFERÊNCIA DE LAJES",
            bg="#1a3d2e",
            fg="white",
            font=("Arial", 14, "bold")
        ).pack(pady=5)
    ...
    def salvar_checklist(self):
        ...
        arquivo = filedialog.asksaveasfilename(
            ...
            initialfile=f"checklist_lajes_{datetime.now().strftime('%Y%m%d_%H%M')}.txt"
        )
...
"""
    ...
    def gerar_romaneio(self):
        ...
        for dado in self.dados_processados:
            ...
            bitola = dado[2]
            comp = dado[4]
            tipo = AnalisadorGeometricoLajes.identificar_tipo_viga(pos, bitola, comp)
    ...
    def _desenhar_topo_identico_fase4(self, x, y, w, h, viga, pos, bitola, qtde, comp):
        ...
        os_txt = f"{indice_global + 1}-{len(self.dados_etiquetas_filtrados)}" if hasattr(self, 'total_paginas') else "-"
        ...
        self.canvas_etiq.create_text(area_util_x1 + mm(24), y_current, text=tipo_exib, font=("Arial", 10, "bold"), anchor="nw")
        y_current += step_y
        ...
        try:
            ...
            try:
                ...
                if not forma:
                    tipo_barra = AnalisadorGeometricoLajes.identificar_tipo_viga(pos, bitola, comp)
    ...
    def editar_tipo_barra_viga(self, idx_barra):
        ...
        # Tipo atual
        tipo_atual = AnalisadorGeometricoLajes.identificar_tipo_viga(pos, bitola, comp)
    ...
    def desenhar_etiquetas_com_selecao(self):
        ...
        # Cálculo de altura total da etiqueta (ORIGINAL) + margem extra
        altura_etiqueta = ALTURA_TOPO + (ESPACO_PICOTE // 2) + 3 * (ALTURA_MICRO + ESPACO_PICOTE) + ALTURA_MICRO
        ...
        for i in range(inicio, fim):
            ...
            for idx in range(3):
                ...
                if hasattr(self, '_desenhar_secao_micro_fase4'):
                    self._desenhar_secao_micro_fase4(x_base + 6 * zf, y_sec + 6 * zf, LARGURA_ETIQ - 12 * zf, ALTURA_MICRO - 12 * zf, viga, pos, bitola, qtde, comp)
    ...
    def _confirmar_e_imprimir_etiquetas(self):
        ...
        # Sincronizar dados do editor antes de imprimir
        self._sincronizar_dados_editor()
