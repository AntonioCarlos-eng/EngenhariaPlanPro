...
print("="*80)
print("TESTE DE VALIDACAO COM QTY <= 2000")
print('='*80)
...
for idx, title in enumerate(table_titles):
    ...
    for linha in linhas:
        ...
        if not (10 <= comp_cm <= 5000):
            rejeicoes.append(f"comp_fora({comp_cm})")
        
        if not (1 <= pos <= 100):
            rejeicoes.append(f"pos_invalida({pos})")
        
        if not (qty > 0 and qty <= 2000):  # NOVA VALIDACAO
            rejeicoes.append(f"qtd_invalida({qty})")
        
        if rejeicoes:
            print(f"  REJEITADA N{pos}: {', '.join(rejeicoes)}")
            total_rejeitadas += 1
        else:
            print(f"  OK N{pos}: BIT={bit} QTY={qty} COMP={comp_cm}")
            total_aceitas += 1

print(f"\n{'='*80}")
print(f"TOTAL: {total_aceitas} aceitas, {total_rejeitadas} rejeitadas")
print('='*80)
