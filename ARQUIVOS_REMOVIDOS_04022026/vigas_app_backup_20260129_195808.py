...
class VigasApp(tk.Tk):
    ...
    def gerar_romaneio(self):
        ...
        tk.Button(print_frame, text="Prévia", command=_do_preview, bg="#eeeeee").grid(row=0, column=6, padx=6)
    ...
    def _editar_desenho_canvas(self, viga, pos):
        ...
        # Diálogo com preview tipo canvas
        dialog = tk.Toplevel(self)
        dialog.title(f"Editar Desenho - {viga} {pos}")
        dialog.geometry("520x420")
        dialog.configure(bg="#ecf0f1")
        dialog.transient(self)
        # Remover grab para não travar tela principal durante edição
        dialog.update_idletasks()
        x_win = (dialog.winfo_screenwidth() // 2) - 260
        y_win = (dialog.winfo_screenheight() // 2) - 210
        dialog.geometry(f"520x420+{x_win}+{y_win}")

        tk.Label(dialog, text=f"Editar Desenho - {viga} {pos}", font=("Arial", 12, "bold"), bg="#ecf0f1").pack(pady=8)

        # Preview
        preview_frame = tk.Frame(dialog, bg="#ecf0f1")
        preview_frame.pack(fill="x", padx=12, pady=6)
        preview_canvas = tk.Canvas(preview_frame, width=350, height=130, bg="#f8fafc", highlightthickness=1, highlightbackground="#cbd5e1")
        preview_canvas.pack()

        # Controles
        controls = tk.Frame(dialog, bg="#ecf0f1")
        ...
        lados_frame = tk.Frame(dialog, bg="#ecf0f1")
        ...
        dobra_frame = tk.Frame(dialog, bg="#ecf0f1")
        ...
        gancho_frame = tk.Frame(dialog, bg="#ecf0f1")
        ...
        def draw_preview():
            ...
            forma = var_forma.get()
            # Render estilo rascunho
            cor = "#1e293b"; esp = 6
            w, h = 350, 130
            if forma == 'estribo':
                x1, y1 = int(w*0.3), int(h*0.2)
                x2, y2 = int(w*0.3+w*0.4), int(h*0.2+h*0.6)
                preview_canvas.create_rectangle(x1, y1, x2, y2, outline=cor, width=esp)
                # Mostrar lados se preenchidos
                try:
                    va = float(ent_a.get()); vb = float(ent_b.get()); vc = float(ent_c.get()); vd = float(ent_d.get())
                    preview_canvas.create_text((x1+x2)//2, y1-8, text=f"A={va:.0f}cm", font=("Arial", 8), fill="#334155")
                    preview_canvas.create_text((x1+x2)//2, y2+8, text=f"C={vc:.0f}cm", font=("Arial", 8), fill="#334155")
                    preview_canvas.create_text(x1-20, (y1+y2)//2, text=f"B={vb:.0f}cm", font=("Arial", 8), fill="#334155")
                    preview_canvas.create_text(x2+20, (y1+y2)//2, text=f"D={vd:.0f}cm", font=("Arial", 8), fill="#334155")
                except Exception:
                    pass
            elif forma == 'estribo_redondo':
                r = int(min(w, h) * 0.3)
                preview_canvas.create_oval(w//2 - r, h//2 - r, w//2 + r, h//2 + r, outline=cor, width=esp)
            elif forma == 'gancho':
                preview_canvas.create_line(int(w*0.1), int(h*0.6), int(w*0.1), int(h*0.35), width=esp, fill=cor)
                preview_canvas.create_line(int(w*0.1), int(h*0.35), int(w*0.85), int(h*0.35), width=esp, fill=cor)
                preview_canvas.create_line(int(w*0.85), int(h*0.35), int(w*0.85), int(h*0.6), width=esp, fill=cor)
                try:
                    vga = float(ent_ga.get()); vgb = float(ent_gb.get())
                    preview_canvas.create_text(int(w*0.1)-15, (int(h*0.6)+int(h*0.35))//2, text=f"A={vga:.0f}cm", font=("Arial", 8), fill="#334155", anchor="e")
                    preview_canvas.create_text(int(w*0.85)+15, (int(h*0.6)+int(h*0.35))//2, text=f"B={vgb:.0f}cm", font=("Arial", 8), fill="#334155", anchor="w")
                except Exception:
                    pass
            elif forma == 'dobra':
                preview_canvas.create_line(int(w*0.1), int(h*0.55), int(w*0.6), int(h*0.55), width=esp, fill=cor)
                preview_canvas.create_line(int(w*0.6), int(h*0.55), int(w*0.6), int(h*0.3), width=esp, fill=cor)
                try:
                    vdobra = float(ent_dobra.get())
                    preview_canvas.create_text(int(w*0.6)+20, int((0.55+0.3)/2*h), text=f"{vdobra:.0f}cm", font=("Arial", 9), fill="#334155", anchor="w")
                except Exception:
                    pass
            else:
                preview_canvas.create_line(int(w*0.1), int(h*0.5), int(w*0.9), int(h*0.5), width=esp, fill=cor)

            # Legenda medidas
            try:
                b = float(ent_bitola.get()); c = float(ent_comp.get()); q = int(ent_qtde.get())
                preview_canvas.create_text(w//2, h-12, text=f"{q}x Ø{b:.1f} • C={c:.2f}m", font=("Arial", 10, "bold"), fill="#334155")
            except Exception:
                pass

        draw_preview()
        for child in controls.winfo_children():
            if isinstance(child, (tk.Entry, tk.Radiobutton)):
                child.bind("<KeyRelease>", lambda e: draw_preview())
        var_forma.trace_add('write', lambda *args: draw_preview())
        ent_dobra.bind("<KeyRelease>", lambda e: draw_preview())
        for child in lados_frame.winfo_children():
            if isinstance(child, tk.Entry):
                child.bind("<KeyRelease>", lambda e: draw_preview())
        for child in gancho_frame.winfo_children():
            if isinstance(child, tk.Entry):
                child.bind("<KeyRelease>", lambda e: draw_preview())
        ...
        tk.Button(btns, text="✗ Cancelar", command=dialog.destroy, bg="#e74c3c", fg="white", font=("Arial", 11, "bold"), width=12).pack(side="left", padx=10)
