...
try:
    ...
    print("[MOTOR] ✅ Usando pilares_motor_dual.py (Dual: Rápido + Completo)")
except Exception as e:
    print(f"[MOTOR] ⚠️ Motor dual indisponível: {e}")
    try:
        from core.pilares_motor import processar_pilares
        print("[MOTOR] ✅ Usando pilares_motor.py (fallback)")
    except Exception:
        print("[MOTOR] ❌ Nenhum motor disponível!")
        pass
try:
        ...
        for i in range(inicio, fim):
            ...
            for idx in range(3):
                ...
                if hasattr(self, '_desenhar_secao_micro_fase4'):
                    self._desenhar_secao_micro_fase4(x_base + 6 * zf, y_sec + 6 * zf, LARGURA_ETIQ - 12 * zf, ALTURA_MICRO - 12 * zf, pilar, pos, bitola, qtde, comp)
