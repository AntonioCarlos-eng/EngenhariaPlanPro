...
from core.lajes_motor import processar_lajes, analisar_barra_geometricamente, BITOLAS_VALIDAS, _calcular_peso
...
class LajesApp(tk.Tk):
    ...
    def processar(self, inicial=False):
        ...
        try:
            ...
            if dados_completos:
                ...
                print(f"OK: {len(dados_completos)} posicoes | {total_pecas} barras | {total_kg:.2f} kg")
    ...
    def gerar_romaneio_conferencia(self):
        """Gera checklist de conferencia para lajes."""
        if not self.dados_processados:
            messagebox.showwarning("Atencao", "Processe os arquivos primeiro!")
            return

        janela_conf = tk.Toplevel(self)
        janela_conf.title("📋 CHECK LIST - Conferencia de Lajes")
        ...
        tk.Label(
            header_frame,
            text="📋 CHECK LIST - CONFERENCIA DE LAJES",
            bg="#1a3d2e",
            fg="white",
            font=("Arial", 14, "bold")
        ).pack(pady=5)
        ...
        tk.Label(
            info_frame,
            text=f"Total: {self.total_pecas} barras | {self.total_kg:.2f} kg",
            bg="#1a3d2e",
            fg="#ff9800",
            font=("Arial", 10, "bold")
        ).pack(side="left", padx=20)
        ...
        headers = ["LAJE", "POS", "BITOLA", "QTD", "COMP", "CORTADO", "DOBRADO", "CONFERIDO", "CARREGADO"]
        ...
        for dado in self.dados_processados:
            ...
            tk.Label(row_frame, text=laje, font=("Arial", 8), width=8).grid(row=0, column=0)
    ...
    def salvar_checklist(self):
        ...
        if arquivo:
            with open(arquivo, 'w', encoding='utf-8') as f:
                f.write("=" * 80 + "\n")
                f.write("CHECK LIST - CONFERENCIA DE LAJES\n")
                f.write("=" * 80 + "\n")
                ...
                f.write(f"{'LAJE':<10} {'POS':<8} {'CORTADO':<10} {'DOBRADO':<10} {'CONFERIDO':<12} {'CARREGADO':<10}\n")
                f.write("-" * 70 + "\n")
        ...
        try:
            ...
            for dado in self.dados_processados:
                if dado[0] != laje_atual:
                    laje_atual = dado[0]
                    linhas_html.append(f'<tr class="laje-header"><td colspan="6">{laje_atual}</td></tr>')
                ...
                linhas_html.append(
                    ...
                    f'<td>{int(round(dado[3]))}</td>'
                    f'<td>{dado[4]:.2f}m</td>'
                    f'<td>{dado[6]:.2f}kg</td>'
                    f'</tr>'
                )
...
<head>
    <meta charset="UTF-8">
    <title>Check List de Lajes</title>
    <style>
        body {{
...
"""
            ...
            html_temp = tempfile.mktemp(suffix="_checklist_lajes.html")
            with open(html_temp, 'w', encoding='utf-8') as f:
                f.write(html_content)

            webbrowser.open('file://' + html_temp)
            print("[CHECK LIST HTML] Aberto no navegador.")
    ...
    def imprimir_com_preview(self, conteudo):
        """Gera HTML do romaneio e abre no navegador para impressão."""
        try:
...
<head>
    <meta charset=\"UTF-8\">
    <title>Romaneio - Impressao</title>
    <style>
        body {{
...
"""
            ...
            html_temp = tempfile.mktemp(suffix="_romaneio_lajes.html")
            with open(html_temp, "w", encoding="utf-8") as f:
                f.write(html_content)

            webbrowser.open("file://" + html_temp)
            print("[IMPRESSAO HTML] Romaneio de lajes aberto no navegador.")
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao gerar impressao: {e}")
