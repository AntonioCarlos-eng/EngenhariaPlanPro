# Classe utilitária para análise geométrica das vigas
...
class VigasApp(tk.Tk):
    ...
    def _abrir_editor_etiquetas(self):
        """Abre a janela do editor visual de etiquetas (padrão lajes)."""
        import traceback
        from tkinter import messagebox
        try:
            if not self.dados_processados:
            ...
            self.janela_editor.title("✏️ EDITOR DE ETIQUETAS - VIGAS")
            ...
            self.label_selecionadas = tk.Label(sel_frame, text=f"Selecionadas: {total_selecionadas}/{len(self.dados_processados)}",
            ...
            try:
                if ETIQUETAS_GERADOR_DISPONIVEL:
                    ...
                    for idx, dado in enumerate(self.dados_processados):
                        viga, pos, bitola, qtde, comp = dado[0], dado[1], dado[2], dado[3], dado[4]
                        png_path = self._gerar_png_etiqueta(idx, viga, pos, bitola, qtde, comp, str(etiq_pasta))
    ...
    def gerar_romaneio_conferencia(self):
        ...
        for i, dado in enumerate(self.dados_processados):
            ...
            # Dados compactos
            tk.Label(row_frame, text=viga, font=("Arial", 8), width=8).grid(row=0, column=0)
        ...
        try:
            ...
            for dado in self.dados_processados:
                if dado[0] != viga_atual:
                    viga_atual = dado[0]
                    linhas_html.append(f'<tr class="viga-header"><td colspan="6">{viga_atual}</td></tr>')
...
<head>
    <meta charset="UTF-8">
    <title>Check List de Vigas</title>
    <style>
        body {{
...
"""
    ...
    def gerar_romaneio(self):
        ...
        total_barras = self.total_barras
        ...
        for dado in self.dados_processados:
            ...
            viga = str(dado[0])
            pos = str(dado[1])
            bitola = _to_float(dado[2])
            qtde = _to_int(dado[3])
            comp = _to_float(dado[4])
            peso = _to_float(dado[5]) if len(dado) > 5 else 0.0
            dados_norm.append((viga, pos, bitola, qtde, comp, peso))

        janela_romaneio = tk.Toplevel(self)
        janela_romaneio.title("Romaneio de Vigas - EngenhariaPlanPro")
        ...
        def imprimir_aba_ativa():
            ...
            if texto_widget:
                ...
                if conteudo.strip():
                    ...
                    messagebox.showwarning("Aviso", "A aba esta vazia!")
            else:
                messagebox.showwarning("Erro", "Nao foi encontrado conteudo para imprimir!")
        ...
        conteudo_geral.append("")
        conteudo_geral.append("=" * 80)
        conteudo_geral.append(f"TOTAL GERAL: {total_barras} barras | {total_kg:.2f} kg")
        conteudo_geral.append("=" * 80)
        ...
        def _safe_tipo(pos, bitola, comp):
            try:
                tipo = AnalisadorGeometricoVigas.identificar_tipo_viga(pos, bitola, comp)
                return tipo if tipo else "DESCONHECIDO"
            except Exception as e:
                print(f"[WARN] Falha ao identificar tipo da viga ({pos}, {bitola}, {comp}): {e}")
                return "DESCONHECIDO"
        ...
        for dado in dados_norm:
            ...
            bitola = dado[2]
            comp = dado[4]
            tipo = _safe_tipo(pos, bitola, comp)
        ...
        conteudo_tipo.append("")
        conteudo_tipo.append("=" * 80)
        conteudo_tipo.append(f"TOTAL GERAL: {total_barras} barras | {total_kg:.2f} kg")
        conteudo_tipo.append("=" * 80)
        ...
        for bitola in sorted(bitolas_agrupadas.keys()):
            ...
            percent = (subtotal_peso / total_kg * 100) if total_kg > 0 else 0
            conteudo_bitola.append(f"    Percentual: {percent:.1f}%")

        conteudo_bitola.append("")
        conteudo_bitola.append("=" * 80)
        conteudo_bitola.append(f"TOTAL GERAL: {total_barras} barras | {total_kg:.2f} kg")
        conteudo_bitola.append("=" * 80)
        ...
        conteudo_resumo.append("-" * 80)
        for bitola in sorted(bitolas_agrupadas.keys()):
            peso_bitola = sum(d[5] for d in bitolas_agrupadas[bitola])
        ...
        conteudo_resumo.append("-" * 80)
        for tipo in sorted(tipos_agrupados.keys()):
            peso_tipo = sum(d[5] for d in tipos_agrupados[tipo])
        ...
        conteudo_resumo.append("")
        conteudo_resumo.append("=" * 80)
        conteudo_resumo.append(f"TOTAL GERAL: {total_barras} barras | {total_kg:.2f} kg")
        conteudo_resumo.append("=" * 80)
    ...
    def desenhar_pagina_etiquetas_vigas(self):
        """Desenha a página atual de etiquetas para vigas usando o layout e dimensões reais do gerador dinâmico."""
    ...
    def desenhar_pagina_etiquetas_vigas_fase4(self):
        ...
        for i in range(inicio, fim):
            ...
            for idx in range(3):
                ...
                self._desenhar_secao_micro_fase4(x_base + 4, y_sec + 2, LARGURA_ETIQ - 8, ALTURA_MICRO - 4, viga, pos, bitola, qtde, comp)
                if idx < 2:
    ...
    def _resolver_forma_medidas(self, viga, pos, bitola, comp):
        """Resolve forma e medidas a partir das customizações e do tipo da barra."""
        chave = (viga, pos)
        ...
        if forma_raw:
            forma_map = {
                ...
                "Dobra Única": "dobra",
                "Estribo": "estribo",
                "Estribo Quadrado": "estribo",
                "Estribo Retângulo": "estribo",
                "Estribo Redondo": "estribo_redondo",
            }
            forma = forma_map.get(forma_raw, str(forma_raw).lower().replace(" ", "_"))
        ...
        if not forma:
            try:
                ...
                if "ESTRIBO" in tipo_upper:
                    ...
                    forma = "dobra"
                elif "POSITIVO" in tipo_upper or "GANCHO" in tipo_upper:
                    forma = "gancho"
                else:
                    forma = "reta"
            except Exception:
    ...
    def _desenhar_moldura_etiqueta(self, x, y, w, h):
        """Desenha moldura com marcas de corte (padrão laje)."""
    ...
    def _desenhar_topo_etiqueta(self, x, y, w, h, viga, pos, bitola, qtde, comp):
        """Desenha o topo (93mm) com informações técnicas (padrão laje)."""
        pxmm = 4
        ...
        self.canvas_etiq.create_text(area_util_x1 + mm(24), y_current, text="VIGA", font=("Arial", 10, "bold"), anchor="nw", fill="#e11d48")
        y_current += step_y
        ...
        elemento_txt = _truncate_text(f"{viga}", elem_max_w, ("Arial", 10, "bold"))
        ...
        if draw_area_x2 > draw_area_x1 and draw_area_y2 > draw_area_y1:
            ...
            self.canvas_etiq.create_rectangle(draw_area_x1, draw_area_y1, draw_area_x2, draw_area_y2,
            ...
            forma, medida_dobra, medidas_gancho, estribo_lados = self._resolver_forma_medidas(viga, pos, bitola, comp)
            self._desenhar_forma_simplificada(self.canvas_etiq, draw_area_x1, draw_area_y1, draw_w, draw_h,
                                              forma, medida_dobra, medidas_gancho, estribo_lados)

    def _desenhar_secao_micro(self, x, y, w, h, viga, pos, bitola, qtde, comp):
        """Desenha seção micro (picote) no padrão laje."""
        obra_nome = self.var_obra.get() if hasattr(self, 'var_obra') else "OBRA 001"
        texto = f"{obra_nome} | {viga} | {pos} | ø{bitola:.1f} | {qtde}x{comp:.2f}m"
    ...
    def _gerar_png_etiqueta_editor(self, idx, viga, pos, bitola, qtde, comp, pasta_destino):
        ...
        draw.text((area_util_x1 + mm(28), y_current), "VIGA", font=font_bold, fill="#e11d48")
        y_current += step_y
        ...
        if draw_area_x2 > draw_area_x1 and draw_area_y2 > draw_area_y1:
            ...
            forma, medida_dobra, medidas_gancho, estribo_lados = self._resolver_forma_medidas(viga, pos, bitola, comp)
            self._desenhar_forma_pil(draw, draw_area_x1, draw_area_y1, draw_area_x2 - draw_area_x1,
                                     draw_area_y2 - draw_area_y1, forma, medida_dobra, medidas_gancho, estribo_lados,
                                     font=font_tiny)
        ...
        for i in range(3):
            ...
            draw.rectangle([(picote_x1, y_picote), (picote_x2, y_picote + altura_picote)], outline="#000000", width=3)
            draw.rectangle([(picote_x1, y_picote), (picote_x2, y_picote + altura_picote)], fill="#f0f0f0")
            texto = f"{obra_nome} | {viga} | {pos} | ø{bitola:.1f} | {qtde}x{comp:.2f}m"
        ...
        nome_arquivo = re.sub(r'[\\/:*?"<>|]', '-', f"{viga}_{pos}_Ø{bitola}_Q{qtde}.png")
    ...
    def _desenhar_topo_identico_fase4(self, x, y, w, h, viga, pos, bitola, qtde, comp):
        ...
        self.canvas_etiq.create_text(area_util_x1, y_current, text="Elemento", font=("Arial", 8), anchor="nw")
        self.canvas_etiq.create_text(area_util_x1 + mm(24), y_current, text=f"{viga}", font=("Arial", 10, "bold"), anchor="nw")
    ...
    def _desenhar_secao_micro_fase4(self, x, y, w, h, viga, pos, bitola, qtde, comp):
        ...
        # Calcular OS por viga (índice/total)
        try:
            # Procurar índice desta etiqueta nos dados processados
            idx = -1
            for i, dado in enumerate(self.dados_processados):
                if dado[0] == viga and str(dado[1]) == str(pos):
                    idx = i
                    break
            
            if idx >= 0:
                viga_index = sum(1 for i in range(idx) if self.dados_processados[i][0] == viga)
        ...
        # Linha 1 (topo): Obra/Pav • Viga/Pos • OS
        y_linha1 = y + h * 0.3
        texto_linha1 = f"{obra}|{pavimento} • {viga}/{pos} • OS:{os_num}"
        self.canvas_etiq.create_text(
    ...
    def _imprimir_etiquetas_rapido(self):
        ...
        # Imprimir todas usando o mesmo fluxo do editor
    ...
    def _imprimir_direto_do_editor(self):
        ...
        try:
            ...
            for idx in selecionadas:
                ...
                viga, pos, bitola, qtde, comp = dado[0], dado[1], dado[2], dado[3], dado[4]
                png_path = self._gerar_png_etiqueta(idx, viga, pos, bitola, qtde, comp, pasta_temp)
    ...
    def desenhar_etiquetas_com_selecao(self):
        """Renderiza etiquetas 100×150mm COM PICOTES + CHECKBOXES (padrão lajes)."""
        try:
            yview_prev = getattr(self, '_pending_yview', None)
        except Exception:
            yview_prev = None
        ...
        for i in range(inicio, fim):
            ...
            self._desenhar_topo_etiqueta(x_base, y_cursor, LARGURA_ETIQ, ALTURA_TOPO, viga, pos, bitola, qtde, comp)
            ...
            for idx in range(3):
                ...
                self._desenhar_secao_micro(x_base + picote_margin + 6 * zf, y_sec + 6 * zf,
                                           LARGURA_ETIQ - picote_margin - 12 * zf,
                                           ALTURA_MICRO - 12 * zf, viga, pos, bitola, qtde, comp)
                if idx < 2:
            ...
            self.canvas_etiq.create_rectangle(x_checkbox-2 * zf, y_checkbox-2 * zf, x_checkbox+checkbox_size+2 * zf,
                                              y_checkbox+checkbox_size+2 * zf, fill="white", outline="white", width=1)
            if self.etiquetas_selecionadas.get(i, True):
                ...
                self.canvas_etiq.create_line(x_checkbox+6*zf, y_checkbox+14*zf, x_checkbox+11*zf, y_checkbox+20*zf,
                                             x_checkbox+22*zf, y_checkbox+5*zf, fill="white", width=int(3*zf),
                                             capstyle="round", joinstyle="round", tags=f"checkbox_{i}")
            else:
                self.canvas_etiq.create_rectangle(x_checkbox, y_checkbox, x_checkbox+checkbox_size, y_checkbox+checkbox_size,
    ...
    def _confirmar_e_imprimir_etiquetas(self):
        """Imprime etiquetas selecionadas no padrão lajes (HTML + PNG base64)."""
        self._imprimir_etiquetas_selecionadas()

    def _imprimir_etiquetas_selecionadas(self):
        """Gera PNGs das etiquetas selecionadas e abre HTML para impressão (padrão lajes)."""
        ...
        try:
            ...
            pasta_temp = os.path.join(tempfile.gettempdir(), "etiquetas_vigas_editor")
            os.makedirs(pasta_temp, exist_ok=True)
            ...
            for idx in selecionadas:
                ...
                caminhos.append(self._gerar_png_etiqueta_editor(idx, viga, pos, bitola_f, qtde_i, comp_f, pasta_temp))
...
<head>
    ...
    <title>Impressão de Etiquetas - Vigas</title>
    <style>
...
"""
            ...
            html_temp = tempfile.mktemp(suffix="_etiquetas_vigas.html")
            with open(html_temp, 'w', encoding='utf-8') as f:
                f.write(html_content)
            try:
                webbrowser.open('file://' + html_temp)
    ...
    def _gerar_png_etiqueta(self, idx, viga, pos, bitola, qtde, comp, pasta_destino):
        """
        ...
        draw.text((LARGURA//2, y), f"OS: {viga}/{pos} {os_num}".strip(), font=font_medium, fill='#000000', anchor='mm')
        ...
        # Viga / Posição (DESTACADO)
        draw.text((LARGURA//2, y), f"{viga} / {pos}", font=font_large, fill='#000080', anchor='mm')
        y += 78
        ...
        # Salvar PNG
        nome_arquivo = f"{viga}_{pos}_Ø{bitola}_Q{qtde}.png"
        caminho_final = os.path.join(pasta_destino, nome_arquivo)
        img.save(caminho_final, dpi=(300, 300))
            ...
            try:
                ...
                for i in range(len(self.dados_processados)):
                    if i in selecionadas:
                        ...
                        dado = self.dados_processados[i]
                        viga, pos, bitola, qtde, comp = dado[0], dado[1], dado[2], dado[3], dado[4]
                        png_path = self._gerar_png_etiqueta(i, viga, pos, bitola, qtde, comp, pasta_temp)
                        self.caminhos_etiquetas_geradas.append(png_path)
                ...
                for idx in selecionadas:
                    ...
                    if idx_lista < len(pngs_preview):
                        ...
                        if os.path.exists(png_path):
                            ...
                            pos_esperada = dado[1]
                            pngs_para_preview.append((idx, png_path, viga_esperada, pos_esperada))
                ...
                for idx, png_path, viga_esperada, pos_esperada in pngs_para_preview:
                ...
                def imprimir_do_preview():
                    ...
                    try:
                        ...
                        pngs_validos = []
                        for idx, png_path, viga, pos in pngs_para_preview:
                            if os.path.exists(png_path):
    ...
    def imprimir_direto(self):
        ...
        try:
            ...
            for dado in self.dados_processados:
                if dado[0] != viga_atual:
                    viga_atual = dado[0]
                    linhas_html.append(f'<tr class="viga-header"><td colspan="5">{viga_atual}</td></tr>')
...
<head>
    <meta charset="UTF-8">
    <title>Relatório de Vigas</title>
    <style>
        body {{
            font-family: Arial, sans-serif;
            margin: 20px;
